...
url = "http://mahaepos.gov.in/fps_complete_abstract.action?dist_code={0}&office_code={1}&year={2}&month={3}"
...
# HEADERS IN THE CSV FILES TO BE ADDED MANUALLY
...
for t in range(12):
    ...
    with open(temp.format(month), 'w') as csvfile:
        ...
        for ind in tqdm.tqdm(range(len(distCodes))):
            # CHANGE YEAR IN THE NEXT LINE
            temp = url.format(str(distCodes[ind]), str(officeCodes[ind]), str(2018), str(month))
